...
export default function MusicPlayer() {
  ...
  return (
    <div
      className={`fixed bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 z-40 animate-slide-up flex flex-col items-center ${
    ...
    >
      {/* Unobtrusive error or status message */}
      {error && (
        <div className="w-full px-5 py-1.5 bg-red-950/60 rounded-t-[60px] border-b border-red-500/20 text-center text-[11px] text-red-200/90 tracking-wide">
          {error}
        </div>
      )}
      ...
      <div className="w-full flex items-center justify-between gap-3 md:gap-5 px-5 md:px-7 py-3 md:py-4">
        ...
        {!isAuthenticated ? (
          <button
            onClick={connect}
            className="shrink-0 px-4 py-2 text-xs font-semibold rounded-full transition-all duration-200 shadow-md active:scale-95 flex items-center gap-2 bg-[#1DB954] hover:bg-[#1ed760] text-black"
          >
