...
export default function MusicPlayer() {
  ...
  return (
    /* Outer container: Fixed 100% width flexbox wrapper guarantees 100% horizontal centering */
    <div className="fixed bottom-6 md:bottom-8 left-0 right-0 z-40 flex items-center justify-center pointer-events-none px-4">
      ...
      <div
        ...
        style={{
          ...
          boxShadow:
            '0 20px 50px rgba(10, 4, 2, 0.65), 0 2px 10px rgba(0, 0, 0, 0.4), inset 0 1px 1px rgba(255, 245, 235, 0.15)',
          fontFamily: 'var(--font-ui)',
        }}
      >
        {/* Unobtrusive error message */}
        {error && (
          <div className="w-full px-5 py-1.5 bg-red-950/80 rounded-t-[60px] border-b border-red-500/20 text-center text-[11px] text-red-200/90 tracking-wide">
            {error}
          </div>
        )}
        ...
        <div className="w-full flex items-center justify-between gap-3 md:gap-4 px-4 md:px-6 py-3 md:py-4">
          ...
          <div className="shrink-0 flex items-center gap-3 md:gap-4">
            {!isAuthenticated ? (
              <button
                onClick={connect}
                className="px-4 md:px-5 py-2.5 text-xs font-semibold rounded-full transition-all duration-200 shadow-[0_4px_16px_rgba(29,185,84,0.3)] active:scale-95 flex items-center gap-2 bg-gradient-to-r from-[#1DB954] to-[#1ed760] hover:brightness-110 text-black font-sans"
              >
